...
今回検証に使用するViteのバージョンは`^5.92.1`です。
...
コードを見ると、`__webpack_require__.a`という関数が使用していることがわかります。この関数は、モジュールを非同期に評価するためのラッパー関数として非同期依存関係の解決を待ち、それが完了した後にモジュールの評価を続行します。

`__webpack_require__.a`の[実装](https://github.com/webpack/webpack/blob/dd44b206a9c50f4b4cb4d134e1a0bd0387b159a3/examples/top-level-await/README.md?plain=1#L239-L271)を追ってみましょう。

まず、非同期モジュールの評価を待機する関数のリストをキューに保持します。`hasAwait`が真の場合、キューを初期化します。
...
次に、非同期モジュールの評価が完了するのを待つために、Promiseを使用して評価の順序を管理します。
...
依存関係がラップされ、それらがPromiseである場合は解決を待ち、それが完了した後にモジュールの評価を続行します。
...
そういう経緯で今回頑張って仕様書を読んでみて、エコシステムとプラットフォーム両方の観点からJavaScriptの世界を見てみました。おわり。
