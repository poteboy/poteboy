...
さて、ここで「ESModulesをそのままブラウザで使用するのはまだ難しいんだね」で終わらせてしまっても良いのですが、本記事ではそこから一歩だけ踏み込んで、ESModulesの仕様からモジュール読み込みの仕組みと課題を理解し、さらにバンドラがESMを取り扱う上で抱える問題は何か、についてちょっとだけ考えてみたいと思います。
...
強連結成分（SCC）とは、有向グラフにおいて、互いに行き来が可能な頂点の集合を指します。 <br />
```
...
1. モジュールの解決（Resolution）
    ...
    - つまり、**モジュールにつき必ずFetchが行われる**ことが分かります。
2. モジュールの取得（Fetching）
...
実際にGoogle Chromeでモジュール実行を検証してみても、期待通りにモジュールが評価されていることが確認できます。
...
依存モジュールの評価は再帰的に行われ、既に評価中のモジュールはスキップされます。
...
さて、ここまでの例では同期的な処理を前提としていましたが、ESModulesには非同期処理を伴うモジュールの評価も存在します。

そう、[Top Level Await（TLA）](https://github.com/tc39/proposal-top-level-await)です。

先ほどまでに確認した通り、ESModulesにおけるモジュールの評価・実行はDFSの帰りがけ順（post-order）で最左端の部分木から行われ、再帰的に親ノードが評価されます。
...
この時、実行順序は前節で示したものと変わりません。ただし、`b.mjs`と`c.mjs`の評価が並列（concurrent）に行われるようになっています。
...
以上により、TLAな世界観においてはモジュールの解決は待機されず、**モジュールの評価が完了する順序は非決定的（non-deterministic）になる**ことが分かりました。


## バンドラでのTop Level Awaitの扱い

ところで、各バンドラではTop Level Await（TLA）を含むモジュールはどのように扱われるのでしょうか。
...
次に、Viteのように開発時にはESModulesをそのまま使用し、本番ビルド時にバンドルするというアプローチを取るバンドラではどうでしょうか。
...
JavaScriptは元来ここまで高速に動く言語ではありませんでした。しかし、ブラウザで動く唯一の言語であるという業を背負っているため、少しでも高速化するために言語仕様制定者や処理系開発者だけでなく、エコシステム側の開発者も日々改善を重ねてきました。

私は、そんなエコシステムの営みが好きです。ただし、ユーザー側でより良い方向にJavaScriptエコシステムを発展させていくためには、仕様を理解しJavaScriptの現在地点を把握することが重要なのかもしれないと考えるようになりました。

そういう経緯で今回頑張って仕様書を読んでみました。でもやっぱりエコシステムの話が好きなのでエコシステムの検証も混ぜました。おわり
